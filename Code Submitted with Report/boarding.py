...
def intalize_render(): 
    ...
    ax.set_yticks(numpy.arange(0.5, NUM_SEATS+len(AISLES)+.5, 1).tolist(), minor=False) 
    ...
    if plane == 'wide wing': 
        ...
        ax.set_yticklabels(['A','B','C','Aisle','D','E','F','G','H','I','Aisle','J','K','L','M','N','O','Aisle','P','Q','R','S','T','U','Aisle','V','W','X']) 

    elif plane == 'narrow body': 
...
def create_boarding_order_for_aisle(boarding_section, other_section1, other_section2, seats): 
...
def cull_boarding_queue(boarding_queue): 
    ...
    #this function has two aims: reduce capacity due to COVID, and remove any seats not included in planes 

     

    # first see if need to cull the seats that would be in grid of planes, but not there 

    # remove them here as easier than having to not add them in the first place in every method 
    ...
    if plane == 'wide wing':    
        ...
        for index,passenger in enumerate(boarding_queue):   
            ...
            # Seats A B C V W X in rows 1-3 

            if passenger[0]-1 in [0,1,2] and passenger[1] in [0,1,2,25,26,27]: 
        ...
        for index,passenger in enumerate(boarding_queue):   
            ...
            if passenger[0]-1 in [0] and passenger[1] in [4,5,6]: 
    ...
    target_to_kill = math.floor((COVID_CAPACITY)*NUM_SEATS) 
...
def seat_boarding_with_groups(): 
    ...
    for _ in range(N_TEST_CASES): 
        ...
        boardingQueue1 = window+middle+aisle 

        for x in boardingQueue1: 

            if x not in boardingQueue: 

                boardingQueue.append(x) 
    ...
    for _ in range(N_TEST_CASES): 
        ...
        for row in range(1,NUM_ROWS+1): 
            ...
            for seat in range(-3,4): 
                ...
                if not is_not_disobedient() and seat != 0: 

                    steffenPerfected[random.randrange(0,2)].append([row,seat+3,assign_luggage(),0]) 

 

                elif (seat > 0): #right side 

                    steffenPerfected[(row%2)*2].append([row,seat+3,assign_luggage(),0])  

                elif (seat < 0): #left side 

                    steffenPerfected[(row%2)*2+1].append([row,seat+3,assign_luggage(),0])  
...
if plane == 'narrow body': 
    ...
    # total loading time for 2E2A plane will be 2E2A + 2E2A first class 
...
NAUGHTY_BOY_COEFFICIENT = 0.18 

COVID_CAPACITY = 0.5 #0, 0.3 0.5 or 0.7 
...
#steffen_modified_method() 

 

#naughty_boy_sensitivity() 
